...
class ControllerService extends BaseNetworkService {
  Rx<ControllerStatus> minionStatus = ControllerStatus.none.obs;
  ...
  void _init() async {
    minionStatus.value = ControllerStatus.initializing;
    ...
    _peerStream = p2p.streamPeers().listen((List<DiscoveredPeers> event) {
      ...
      if (minionStatus.value != ControllerStatus.active) {
        minionStatus.value = ControllerStatus.searchingBoss;
        _peers = event;
  ...
  void initiateConnectionToGru(DiscoveredPeers gru) {
    ...
    minionStatus.value = ControllerStatus.connectingBoss;
  ...
  Future connectToSocket(WifiP2PInfo info) async {
    minionStatus.value = ControllerStatus.connectingSocket;
    ...
    while (!succeeded) {
      succeeded = await p2p.connectToSocket(
        ...
        onConnect: (String address) {
          _connected = true;
          minionStatus.value = ControllerStatus.active;
